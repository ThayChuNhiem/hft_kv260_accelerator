...
**Mục tiêu Công bố Học thuật**: Bài báo Khoa học Quốc tế Q1/Q2 (IEEE Transactions / ACM / Scopus / ISI)  
**Cập nhật gần nhất**: 2026-08-14  

---

## 🎓 ĐỊNH HƯỚNG BÀI BÁO KHOA HỌC CHUẨN IEEE / ACM Q1/Q2

### 🏷️ Tên Bài Báo Đề Xuất (Paper Title):
> **"A Sub-Microsecond Hybrid HW/SW Co-Design HFT Accelerator Architecture on AMD Kria KV260 FPGA with Application to Index-Futures Arbitrage"**  
> *(Kiến trúc Tăng tốc HFT Hybrid HW/SW Co-Design Độ trễ Sub-Microsecond trên AMD Kria KV260 FPGA với Ứng dụng cho Kinh doanh Chênh lệch Giá Phái sinh - Chỉ số)*

---

### 📐 Dàn Ý 6 Phần Chuẩn Bài Báo Q1/Q2 (Journal Paper Structure):

1. **I. INTRODUCTION (Giới thiệu)**:
   - Thách thức độ trễ nanosecond trong giao dịch tần số cao (HFT).
   - Hạn chế của phần mềm C++ (Kernel network stack latency $1.5\text{ }\mu\text{s} - 15\text{ }\mu\text{s}$).
   - Đóng góp chính của bài báo: Kiến trúc Co-Design độ trễ $105\text{ ns}$ trên kit AMD Kria KV260.
2. **II. SYSTEM ARCHITECTURAL DESIGN (Thiết kế Kiến trúc Hệ thống)**:
   - Trình bày mô hình phân vùng Co-Design:
     - **Data Plane (PL FPGA)**: Zero-copy Fast Path (`itch_parser` $\rightarrow$ `matching_engine` $\rightarrow$ `ouch_formatter`).
     - **Control Plane (PS ARM)**: Slow Path & Telemetry Log qua AXI DMA & PYNQ.
3. **III. HARDWARE IMPLEMENTATION DETAILS (Chi tiết Mã nguồn RTL Phần cứng)**:
   - Trình bày FSM giải mã NASDAQ ITCH 5.0 (`itch_parser.sv`).
   - Trình bày Sổ lệnh L3 BRAM & Cập nhật BBO $\le 2$ cycles ($10\text{ ns}$) (`matching_engine.sv`).
   - Trình bày bộ đóng gói lệnh OUCH 4.2 ($35\text{ ns}$) (`ouch_formatter.sv`).
4. **IV. PROPOSED HFT STRATEGY FRAMEWORK (Khung Chiến lược Giao dịch)**:
   - Toán học thuật toán phát hiện chênh lệch giá ($P_{\text{Bid}} \ge P_{\text{Ask}}$).
5. **V. EXPERIMENTAL RESULTS & CASE STUDIES (Kết quả Thực nghiệm & Case Study Minh họa)**:
   - **5.1 Latency Benchmark**: Bảng đo đạc chi tiết $105.0\text{ ns}$ (so sánh với C++ DPDK $1,200\text{ ns}$).
   - **5.2 Resource Utilization**: Bảng tiêu thụ tài nguyên FPGA (LUT, FF, BRAM, DSP, Power trên ZU5EV).
   - **5.3 Real-World Case Study (VN30 Futures & Warrants Arbitrage)**: Minh họa ứng dụng kiến trúc này để ăn chênh lệch giá giữa Hợp đồng tương lai VN30F1M trên HNX và Danh mục 30 cổ phiếu VN30 cơ sở (Thị trường phái sinh T+0).
6. **VI. CONCLUSION & FUTURE WORK (Kết luận & Hướng phát triển)**.
...
2. **Index-Futures & Warrants Arbitrage (Chênh lệch giá Phái sinh VN30 / Emerging Markets)**:
   - **Cơ chế**: Tính toán chỉ số VN30 cơ sở từ 30 cổ phiếu thành phần (bằng FPGA trong $50\text{ ns}$).
   - **Thực thi**: Khi 30 cổ phiếu VN30 tăng giá nhưng Hợp đồng phái sinh VN30F1M (T+0) trên HNX chưa kịp nhảy giá, FPGA bắn lệnh Mua phái sinh đón đầu!

3. **High-Frequency Market Making (Tạo thanh khoản siêu tốc)**:
...
│   │   ├── hft_top.sv                  # [Top Module] Module tích hợp trọn gói toàn bộ luồng phần cứng Fast Path
...
│   │   ├── tb_matching_engine.sv       # [Matching Testbench] Mô phỏng cập nhật sổ lệnh BRAM & thời gian cập nhật BBO <= 2 cycles
│   │   ├── tb_ouch_formatter.sv        # [OUCH Testbench] Mô phỏng phát gói lệnh OUCH 4.2 Enter Order 49 Bytes
│   │   ├── tb_hft_top.sv               # [Full Pipeline Testbench] Mô phỏng tích hợp toàn bộ hệ thống & đo đạc Latency 105ns
...
│   └── scripts/
│       ├── sim_parser.tcl              # [Vivado Automation] Script Tcl tự động khởi tạo project Vivado, chạy mô phỏng ITCH Parser
│       ├── sim_matching.tcl            # [Vivado Automation] Script Tcl tự động chạy mô phỏng Matching Engine
│       ├── sim_ouch.tcl                # [Vivado Automation] Script Tcl tự động chạy mô phỏng OUCH Formatter
│       └── sim_top.tcl                 # [Vivado Automation] Script Tcl tự động chạy mô phỏng Full System Top Pipeline
...
| 3 | `hft_pkg.sv` | [file](file:///d:/2026/FPGA/hft_kv260_accelerator/hw/src/common/hft_pkg.sv) | SystemVerilog Package: Chứa MoldUDP64 Struct, 10 loại ITCH 5.0 Struct, BBO Struct, OUCH Struct và Enums |
...
| 6 | `ouch_formatter.sv` | [file](file:///d:/2026/FPGA/hft_kv260_accelerator/hw/src/ouch_formatter/ouch_formatter.sv) | SystemVerilog Formatter: Đóng gói lệnh OUCH 4.2 ('O') 49 Bytes truyền ra AXI-Stream trong $35\text{ ns}$ |
| 7 | `hft_top.sv` | [file](file:///d:/2026/FPGA/hft_kv260_accelerator/hw/src/hft_top.sv) | SystemVerilog Top Level Wrapper: Đấu nối trọn gói toàn bộ pipeline phần cứng Network In -> ITCH -> Matching -> OUCH -> Network Out |
| 8 | `tb_itch_parser.sv` | [file](file:///d:/2026/FPGA/hft_kv260_accelerator/hw/tb/tb_itch_parser.sv) | Testbench đơn vị (Unit Test) cho bộ giải mã ITCH 5.0 |
| 9 | `tb_itch_parser_full.sv` | [file](file:///d:/2026/FPGA/hft_kv260_accelerator/hw/tb/tb_itch_parser_full.sv) | Testbench tự kiểm tra toàn diện 10 loại message ITCH 5.0 từ file `.mem` |
| 10 | `tb_matching_engine.sv` | [file](file:///d:/2026/FPGA/hft_kv260_accelerator/hw/tb/tb_matching_engine.sv) | Testbench kiểm thử BBO Top-of-Book và tín hiệu Trade Trigger cho Matching Engine |
| 11 | `tb_ouch_formatter.sv` | [file](file:///d:/2026/FPGA/hft_kv260_accelerator/hw/tb/tb_ouch_formatter.sv) | Testbench kiểm thử phát lệnh OUCH 4.2 và giải mã stream 64-bit |
| 12 | `tb_hft_top.sv` | [file](file:///d:/2026/FPGA/hft_kv260_accelerator/hw/tb/tb_hft_top.sv) | Full Integrated System Testbench: Đo đạc thời gian Tick-to-Trade Latency chuẩn nanoseconds ($105\text{ ns}$) |
| 13 | `itch_data_dump.mem` | [file](file:///d:/2026/FPGA/hft_kv260_accelerator/hw/tb/itch_data_dump.mem) | Mảng dữ liệu Hex 64-bit của gói MoldUDP64/ITCH 5.0 nạp vào XSIM |
| 14 | `sim_parser.tcl` | [file](file:///d:/2026/FPGA/hft_kv260_accelerator/hw/scripts/sim_parser.tcl) | Vivado Tcl script tự động hóa chạy mô phỏng ITCH Parser |
...
- [x] **Tuần 7**: Mô phỏng toàn bộ luồng phần cứng: `Network In -> ITCH Parser -> Matching Engine -> OUCH Out` qua Top Module [hft_top.sv](file:///d:/2026/FPGA/hft_kv260_accelerator/hw/src/hft_top.sv) và Testbench [tb_hft_top.sv](file:///d:/2026/FPGA/hft_kv260_accelerator/hw/tb/tb_hft_top.sv) ($105.0\text{ ns}$ latency). *(ĐÃ HOÀN THÀNH 100%)*
...
- [x] **Tuần 10**: Hoàn thiện Vivado Build Script [build_project.tcl](file:///d:/2026/FPGA/hft_kv260_accelerator/hw/scripts/build_project.tcl) và file gán chân / thời gian [kria_kv260.xdc](file:///d:/2026/FPGA/hft_kv260_accelerator/hw/constraints/kria_kv260.xdc) để tự động hóa quá trình Synthesize, Place & Route, tạo file Bitstream (`.bit`) và Hardware Platform Definition (`.xsa`). *(ĐÃ HOÀN THÀNH 100%)*
...
- [ ] **Tuần 13**: Thực hiện **Tick-to-Trade Latency Benchmarking**: Dùng Hardware 64-bit Timer đếm thời gian từ chu kỳ `rx_tvalid` đầu tiên đến `tx_tvalid` phát lệnh (Mục tiêu Latency $< 500\text{ ns}$).

---

#### 🟣 GIAI ĐOẠN 5: Viết Báo Cáo IEEE/ACM & Hoàn Thiện Dự Án (Tuần 14 – Tuần 16)
> **Mục tiêu**: Đóng gói kết quả nghiên cứu thành bài báo khoa học chuẩn IEEE/ACM Q1/Q2 format.

- [ ] **Tuần 14**: Tổng hợp số liệu tài nguyên FPGA (LUT, BRAM, DSP, Power trên ZU5EV) và vẽ biểu đồ phân bố độ trễ (Latency Histogram).
- [ ] **Tuần 15**: Viết bài báo bằng LaTeX trong thư mục [docs/ieee_paper/](file:///d:/2026/FPGA/hft_kv260_accelerator/docs/ieee_paper/): Khung kiến trúc tổng quát + Case study VN30 Futures / Warrants Arbitrage.
- [ ] **Tuần 16**: Rà soát toàn bộ mã nguồn và phát hành bản phát hành chính thức trên GitHub (`ThayChuNhiem/hft_kv260_accelerator`).
